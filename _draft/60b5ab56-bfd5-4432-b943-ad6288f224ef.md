...
tags: ["tool::nmap", "topic::2024-03-xz-utils-backdoor"]
keywords: ["rolling-release", "t64 移行 (Debian)", "2038年問題", "OS::Debian", "security", "desktop-environment::Xfce", "ソフトウェア配信::ミラーリング？", "ソフトウェア配信::CDN？", "wallpaper", "Kali-Undercover"]
...
- > 彼らの協力のおかげで、Kali イメージは現在、米国に 7 つ、コロンビアに 1 つ、英国に 1 つ、オーストラリアに 1 つ、合計10 個の追加ミラーから提供されています。(注：機械翻訳)
  - 残念ながらアジアは無い
- Ref:
  - [micromirrors/doc/product-brief.md · PhirePhly/micromirrors](https://github.com/PhirePhly/micromirrors/blob/909a619d4489bbbc1fae7053852dba79f1c1660a/doc/product-brief.md)
- 余談：
...
- Xfce:
  - Kali-Undercover モードでアップデートがあった
    - [Kali　Undercover]((https://www.kali.org/docs/introduction/kali-undercover/)) モードとは？
      ...
      - アンダーカバー モードへの切り替えは非常に簡単で、コマンド `$ kali-undercover` を実行するだけ。または、デスクトップのメニューから「Kali Undercover Mode」を探して、そこから起動する。元に戻すには、前のコマンドを再度入力するだけ
...
- 2024.1以降、`xz-utils` 周辺では様々な動きがあった。そのため、関連するブログ記事が投稿された：
